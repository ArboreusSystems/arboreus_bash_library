...
export secp256k1_CFLAGS="-I${SECP256K1_PREFIX_ANDROID_X86_64}/include";
export secp256k1_LIBS="-L${SECP256K1_PREFIX_ANDROID_X86_64}/lib -lsecp256k1";


# -----------------------------------
# Functionality

function mLibbitcoinSystemEchoEnvironmentForAndroid_X86_64() {
	...
	echo "LIBBITCOIN_SYSTEM_PREFIX_ANDROID_X86_64: ${LIBBITCOIN_SYSTEM_PREFIX_ANDROID_X86_64}";
	mEchoSeparator;
}

function mLibbitcoinSystemCleanForAndroid_X86_64() {

	mCleanDir ${LIBBITCOIN_SYSTEM_DIR_SOURCES_WITH_NAME};
	mCleanDir ${LIBBITCOIN_SYSTEM_PREFIX_ANDROID_X86_64};
	mEchoSeparator
}

function mLibbitcoinSystemConfigureForAndroid_X86_64() {

	source ${A_DIR_SCRIPTS}/Global/GlobalEnvironment_Android_x86_64.sh;
	mEchoSeparator;

	echo "Configuring for Android X86_64 build";
	cd ${LIBBITCOIN_SYSTEM_DIR_SOURCES_WITH_NAME};

	./autogen.sh;
	./configure \
	    --prefix="${LIBBITCOIN_SYSTEM_PREFIX_ANDROID_X86_64}" \
	    --host="${TARGET}" \
	    --enable-static \
	    --disable-shared \
	    --with-boost=${BOOST_PREFIX_ANDROID_X86_64} \
		--with-icu=${ICU_PREFIX_ANDROID_X86_64} \
		--with-tests=no \
		--with-examples=no;

	echo "Libbitcoin configuring finished for Android X86_64";
	mEchoSeparator;
}

function mLibbitcoinSystemBuildForAndroid_X86_64() {

	cd ${LIBBITCOIN_SYSTEM_DIR_SOURCES_WITH_NAME};
	make -j8;
	make install;
	echo "Libbitcoin built successfully for Android X86_64.";
	mEchoSeparator;
}
...
mLibbitcoinSystemEchoEnvironmentForAndroid_X86_64;
mLibbitcoinSystemCleanForAndroid_X86_64;
mLibbitcoinSystemGetSources;
mLibbitcoinSystemConfigureForAndroid_X86_64;
mLibbitcoinSystemBuildForAndroid_X86_64;
