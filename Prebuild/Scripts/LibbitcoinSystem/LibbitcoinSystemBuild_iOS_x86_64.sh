...
function mLibbitcoinSystemEchoEnvironmentForIOS_x86_64() {
	...
	echo "LIBBITCOIN_SYSTEM_PREFIX_IOS_X86_64: ${LIBBITCOIN_SYSTEM_PREFIX_IOS_X86_64}";
	mEchoSeparator;
}

function mLibbitcoinSystemCleanForIOS_x86_64() {

	mCleanDir ${LIBBITCOIN_SYSTEM_DIR_SOURCES_WITH_NAME};
	mCleanDir ${LIBBITCOIN_SYSTEM_PREFIX_IOS_X86_64};
	mEchoSeparator
}

function mLibbitcoinSystemConfigureForIOS_x86_64() {

	export ARCH=x86_64;
	export PREFIX=${LIBBITCOIN_SYSTEM_PREFIX_IOS_X86_64};
	source ${A_PREBUILT_LIBS_ROOT}/Scripts/Global/GlobalEnvironment_iOS.sh;
	mEchoSeparator;

	echo "Configuring for IOS x86_64 build";
	cd ${LIBBITCOIN_SYSTEM_DIR_SOURCES_WITH_NAME};

	./autogen.sh;
	./configure \
		--host=${ARCH} \
		--prefix=${LIBBITCOIN_SYSTEM_PREFIX_IOS_X86_64} \
		--enable-static \
		--disable-shared \
	...
	echo "Libbitcoin configuring finished for iOS x86_64";
	mEchoSeparator;
}

function mLibbitcoinSystemBuildForIOS_x86_64() {

	cd ${LIBBITCOIN_SYSTEM_DIR_SOURCES_WITH_NAME};
	make -j8;
	make install;
	echo "Libbitcoin built successfully for iOS x86_64.";
	mEchoSeparator;
}
...
mLibbitcoinSystemEchoEnvironmentForIOS_x86_64;
mLibbitcoinSystemCleanForIOS_x86_64;
mLibbitcoinSystemGetSources;
mLibbitcoinSystemConfigureForIOS_x86_64;
mLibbitcoinSystemBuildForIOS_x86_64;
